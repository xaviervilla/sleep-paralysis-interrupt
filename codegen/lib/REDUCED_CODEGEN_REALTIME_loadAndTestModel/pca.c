...
void pca(const double x[16], double varargout_1_data[], int varargout_1_size[2],
         double varargout_2_data[], int varargout_2_size[2])
{
  ...
  for (minDOFnsv = 0; minDOFnsv < 16; minDOFnsv++) {
  ...
  if (noNaNs) {
    ...
    varargout_1_size[0] = 8;
    varargout_1_size[1] = DOF;
    if (0 <= DOF - 1) {
      memcpy(&varargout_1_data[0], &coeff_data[0], 8U * sizeof(double));
    }
  } else {
    minDOFnsv = 2 - irow;
    irow = -1;
