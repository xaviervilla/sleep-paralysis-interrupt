              ...
              [2], double S_data[], int S_size[1], double V_data[], int V_size[2])
{
  ...
  double b;
  n = A_size[0];
  qp1q = static_cast<signed char>((A_size[0] + 1));
  if (0 <= qp1q - 1) {
    std::memset(&s_data[0], 0, qp1q * sizeof(double));
  }

  std::memset(&e[0], 0, 128U * sizeof(double));
  qp1q = static_cast<signed char>(A_size[0]);
  if (0 <= qp1q - 1) {
    std::memset(&work_data[0], 0, qp1q * sizeof(double));
  }

  U_size[0] = static_cast<signed char>(A_size[0]);
  U_size[1] = static_cast<signed char>(A_size[0]);
  qp1q = static_cast<signed char>(A_size[0]) * static_cast<signed char>(A_size[0]);
  if (0 <= qp1q - 1) {
    std::memset(&U_data[0], 0, qp1q * sizeof(double));
  }
  ...
  if (A_size[0] == 0) {
    ...
    for (q = 0; q < qp1q; q++) {
      ...
      if (q + 1 <= n) {
        ...
        if (nrm == 0.0) {
          ...
          if (q + 2 <= n) {
            if (qp1 <= n) {
              std::memset(&work_data[qp1 + -1], 0, ((n - qp1) + 1) * sizeof
                          (double));
            }
    ...
    for (q = 127; q >= 0; q--) {
      ...
      std::memset(&Vf[q * 128], 0, 128U * sizeof(double));
      Vf[q + (q << 7)] = 1.0;
    }
  ...
  for (k = 0; k < n; k++) {
    S_data[k] = s_data[k];
    std::memcpy(&V_data[k * 128], &Vf[k * 128], 128U * sizeof(double));
  }
}
...
void xzsvdc(double A[2048], double U[256], double S[16], double V[2048])
{
  ...
  double sqds;
  double b;
  std::memset(&s[0], 0, 17U * sizeof(double));
  std::memset(&e[0], 0, 128U * sizeof(double));
  std::memset(&work[0], 0, 16U * sizeof(double));
  std::memset(&U[0], 0, 256U * sizeof(double));
  ...
  s[16] = 0.0;
  e[16] = 0.0;
  std::memset(&U[240], 0, 16U * sizeof(double));
  U[255] = 1.0;
  for (q = 14; q >= 0; q--) {
    qp1 = q + 2;
    iter = q << 4;
    ...
    if (s[q] != 0.0) {
      ...
      std::memset(&U[iter], 0, 16U * sizeof(double));
      U[qq] = 1.0;
    }
  }
  ...
  for (k = 0; k < 16; k++) {
    S[k] = s[k];
    std::memcpy(&V[k * 128], &Vf[k * 128], 128U * sizeof(double));
  }
}
